...
SERVER_PORT_MAP = {'Alford':8010,'Ball':8011,'Hamilton':8012,'Holiday':8013,'Welsh':8014}
...
class ServerHerdTest(unittest.TestCase):
    ...
    def test_basic_location_propagation(self):
        ...
        time.sleep(5) # allow some time for location information to be propagated
        second_at = do_communicate('Alford', b'WHATSAT %s 1 1\n' % (client_id.encode())).split(b'\n')[0].rstrip()
        self.assertEqual(second_at, first_at, "Alford should answer WHATSAT queries using location info from Alford")
